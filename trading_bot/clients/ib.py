...
class IBapi(EWrapper, EClient):
    ...
    def tickByTickMidPoint(self, reqId: int, time: int, midPoint: float):
        ...
        self.ticks_data[reqId] = {'ltp_time': datetime.fromtimestamp(int(time)).astimezone(TZ), 'ltp': midPoint}

    def historicalData(self, reqId, bar):

        date_str = bar.date if len(bar.date.split()) < 2 else bar.date.split()[0] + ' ' + bar.date.split()[1]
        data = {'datetime': parse(date_str).astimezone(TZ), 'open': bar.open,
    ...
    def tickOptionComputation(self, reqId, tickType, foo, impliedVol: float, delta: float, optPrice: float,
                              pvDividend: float, gamma: float, vega: float, theta: float, undPrice: float):
        super().tickOptionComputation(reqId, tickType, foo, impliedVol, delta, optPrice, pvDividend, gamma, vega,
                                      theta, undPrice)
        if delta is not None and optPrice is not None:
            data = {'price': optPrice, 'delta': delta, 'id': reqId}
            self.option_computations[reqId] = data
    ...
    def create_combo_leg_contract(self, symbol, contracts):
        all_contracts = dict()
        for c in contracts:
            self.nextorderId += 1
            req_id = self.nextorderId
            self.reqContractDetails(req_id, c)
            all_contracts[req_id] = c

        combo_contract = self.make_contract(symbol, 'BAG', 'SMART', 'ISLAND')
        combo_contract.comboLegs = []
        for i, c in all_contracts.items():
            while not self.contract_data.get(i):
                pass
            contract = self.contract_data.get(i).contract
            logger.info(f'{symbol}, contract details fetched, contract: {contract}')
